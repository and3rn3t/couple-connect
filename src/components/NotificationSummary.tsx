...
export default function NotificationSummary({
  ...
  const totalNotifications =
    overdueActions.length + dueSoonActions.length + recentCompletions.length;
      ...
      <CardContent className="space-y-3">
        {overdueActions.length > 0 && (
          <div className="flex items-center justify-between p-3 bg-destructive/10 rounded-md">
            ...
            <Badge variant="destructive">{overdueActions.length}</Badge>
          </div>
        )}
        ...
        {dueSoonActions.length > 0 && (
          <div className="flex items-center justify-between p-3 bg-accent/10 rounded-md">
            ...
            <Badge variant="secondary">{dueSoonActions.length}</Badge>
          </div>
        )}
        ...
        {overdueActions.length > 0 && (
          <div className="pt-2 border-t">
            ...
            <p className="text-sm font-medium">{overdueActions[0].title}</p>

...
export function useNotificationSystem({
  ...
  const [lastNotificationCheck, setLastNotificationCheck] = useKV<string>('last-notification-check', '')
    ...
    actions.forEach(action => {
      ...
      if (settings.overdueReminders && daysDiff < 0) {
        ...
        if (settings.browserNotifications && 
            ...
            Math.abs(daysDiff) <= 3) { // Only for recently overdue items
  ...
  }, [actions, issues, currentPartner.id, settings, lastNotificationCheck, setLastNotificationCheck])

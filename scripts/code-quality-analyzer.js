...
class CodeQualityAnalyzer {
  ...
  async scanFiles() {
    ...
    const files = this.getAllFiles(this.srcDir).filter((file) => this.isSourceFile(file));
    ...
    this.analysis.summary.totalLines = this.analysis.files.reduce(
      (sum, file) => sum + file.lines,
      0
    );

    console.log(
      `📊 Scanned ${files.length} files (${this.analysis.summary.totalLines.toLocaleString()} lines)`
    );
    console.log();
  }
  ...
  async detectTechnicalDebt() {
    ...
    const debtPatterns = [
      ...
      {
        pattern: /componentWillMount|componentWillReceiveProps/g,
    ...
    console.log(
      `⚠️  Debt issues found: ${this.analysis.issues.filter((i) => i.type === 'technical-debt').length}`
    );
    console.log();
  }
  ...
  async checkTypeScriptCompliance() {
    ...
    const tsFiles = this.analysis.files.filter(
      (file) => file.extension === '.ts' || file.extension === '.tsx'
    );
  ...
  async validateReactPatterns() {
    ...
    const reactFiles = this.analysis.files.filter(
      (file) =>
        file.extension === '.tsx' ||
        file.extension === '.jsx' ||
  ...
  generateRecommendations() {
    ...
    if (issuesByType['infinite-loop-risk'] > 0) {
      recommendations.push(
        '🚨 CRITICAL: Fix infinite loop risks immediately to prevent blank screens'
      );
    }
...
${Object.entries(
  ...
  .sort(([, a], [, b]) => b - a)
  .slice(0, 5)
  .map(([type, count]) => `- **${type.replace('-', ' ')}**: ${count} issues`)
  .join('\n')}

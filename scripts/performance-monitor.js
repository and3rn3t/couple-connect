...
class PerformanceMonitor {
  ...
  async analyzeWebVitals() {
    ...
    vitalsMetrics.scores.lcp =
      vitalsMetrics.lcp < 2500 ? 'good' : vitalsMetrics.lcp < 4000 ? 'needs-improvement' : 'poor';
    vitalsMetrics.scores.fid =
      vitalsMetrics.fid < 100 ? 'good' : vitalsMetrics.fid < 300 ? 'needs-improvement' : 'poor';
    vitalsMetrics.scores.cls =
      vitalsMetrics.cls < 0.1 ? 'good' : vitalsMetrics.cls < 0.25 ? 'needs-improvement' : 'poor';
    vitalsMetrics.scores.fcp =
      vitalsMetrics.fcp < 1800 ? 'good' : vitalsMetrics.fcp < 3000 ? 'needs-improvement' : 'poor';
  ...
  async saveResults(results) {
    ...
    const reportFile = resolve(
      this.reportsDir,
      `performance-${this.timestamp.replace(/[:.]/g, '-')}.json`
    );
  ...
  generateRecommendations(results) {
    ...
    if (results.build.duration > 15000) {
      recommendations.push(
        'Optimize build time by enabling incremental builds and improving caching'
      );
    }
    ...
    if (results.bundle.javascript / results.bundle.total > 0.8) {
      recommendations.push(
        'JavaScript bundle is too large - consider lazy loading and vendor splitting'
      );
    }
    ...
    if (results.vitals.scores.lcp !== 'good') {
      recommendations.push(
        'Improve Largest Contentful Paint by optimizing images and critical CSS'
      );
    }
...
${
  report.details.regression.length > 0
    ? report.details.regression
        .map((reg) => `- **${reg.type}**: ${reg.message} (${reg.severity} severity)`)
        .join('\n')

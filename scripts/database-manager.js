...
class DatabaseManager {
  ...
  async checkMigrationStatus() {
    ...
    try {
      const migrationFiles = execSync(`ls "${this.migrationsDir}"/*.sql 2>/dev/null || echo ""`, {
        encoding: 'utf8',
      })
        .split('\n')
  ...
  async analyzePerformance() {
    ...
    if (existsSync(schemaPath)) {
      try {
        ...
        console.log(
          `📊 Schema: ${(performanceMetrics.schemaSize / 1024).toFixed(1)}KB, ${tableCount} tables`
        );
      } catch (error) {
    ...
    if (existsSync(seedPath)) {
      try {
        ...
        console.log(
          `📊 Seed data: ${(performanceMetrics.seedDataSize / 1024).toFixed(1)}KB, ${insertCount} inserts`
        );
      } catch (error) {
    ...
    if (existsSync(this.backupsDir)) {
      try {
        const backupFiles = execSync(
          `ls -lt "${this.backupsDir}"/*.sql 2>/dev/null | head -1 || echo ""`,
          { encoding: 'utf8' }
        );
        if (backupFiles.trim()) {
